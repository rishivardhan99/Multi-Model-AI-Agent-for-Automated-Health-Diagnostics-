...
Produces:
  ...
  - outputs/model2_outputs/raw_input_<basename> (copy)
...
from .serializer import save_json, save_text
...
def run(input_path: str, output_dir: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 3. Build observations for KG/probable causes
        observations = []
        # from detected patterns' support lists
        for pname, pinfo in patterns.get("patterns", {}).items():
            if pinfo.get("present"):
                # support entries should already be observation-like tokens such as "Platelets_LOW"
        ...
        # 9. Sanitize (guardrails)
        safe_output = sanitize_output(output)

        # 10. Persist outputs inside model2_outputs
